...
def dection():
    ...
    count=0
    for file in os.listdir(img_dir):
        facePath = os.path.join(emb_dir,file)
        misc.imsave(facePath,images[count])
        count=count+1
        insertService.InsertFaceInfo(file.split("_")[0],file.split("_")[1],'/upload/'+file.filename)

def detectSinglePicture(Dir,fileUrl):
    single_img=os.path.join(Dir,fileUrl)
    print('loading...... :'+single_img)
    img_path_set=[]
    img_path_set.append(single_img)
    images = load_and_align_data(img_path_set, 160, 44, 1.0)
    ...
    # 存臉部
    misc.imsave(os.path.join(emb_dir,fileUrl),images[0])
